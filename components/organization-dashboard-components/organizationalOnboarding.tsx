...
//     setValue('organization_image', base64String); // ✅ store base64 string instead of File
//   };
//   reader.readAsDataURL(file);
// }
...
// ✅ Helper to check for Base64 image
const isBase64Image = (val: any) =>
  typeof val === 'string' &&
  /^data:image\/(png|jpeg|jpg|webp);base64,/.test(val);
...
const validationSchema = yup.object({
  ...
  organization_image: yup
    .mixed()
    .required('Profile image is required')
    .test('is-valid-image', 'Only image files are allowed', (value: any) => {
...
const OrganizationalDetails = ({ onClose }: OrganizationalDetailsProps) => {
  ...
  // ✅ File handler (converts to Base64)
  function handleFileChange(e: ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0];
    if (!file) return;
    ...
    reader.onload = () => {
      ...
      setValue('organization_image', base64String, {
        shouldValidate: true,
        shouldDirty: true,
      });
    };
  ...
  const submit = (data: any) => {
    // ✅ Send Base64 string directly to API
    onboarding.mutate(data, {
      onSuccess: () => {
        toast.success('Onboarding completed successfully!');
        setShowSuccessModal(true);
  ...
  return (
    <div className="mx-auto w-[576px] p-10">
      ...
      <form className="space-y-4" onSubmit={handleSubmit(submit)}>
        ...
        <div>
          <Label htmlFor="SM-links" className="mb-4 font-normal">
            ...
            <span className="text-xs text-gray-500">
              (Used to verify your organization&apos;s online presence)
            </span>
          </Label>
          <div className="flex flex-col gap-4">
            <Input
        ...
        <div className="mt-10">
          <Button
            className="bg-iq-500 hover:bg-iq-500 h-auto w-full rounded-md px-6 py-3 text-white"
            type="submit"
            disabled={onboarding.isPending}
          >
            {onboarding.isPending
