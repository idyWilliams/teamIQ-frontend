				...
				<div className="grid grid-cols-5 gap-6">
					{teamMembers.map((member) => (
						<div
							...
							className="flex flex-col items-center p-6 border-2 border-gray-300 rounded-lg bg-white cursor-pointer hover:border-blue-500 hover:shadow-lg transition-all"
						>
							<Image
								src={member.avatar}
								alt={member.name}
							...
							<div className="text-center">
								...
								<div className="flex items-center justify-center gap-1.5">
									...
									<span className="text-sm text-blue-600">{member.role}</span>
								</div>
							</div>
						</div>
					))}
				...
				<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
					<div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto relative">
						...
						<button
							...
							className="absolute top-6 right-6 w-10 h-10 bg-gray-900 text-white rounded-full flex items-center justify-center hover:bg-gray-800 z-10"
						>
									...
									<div className="mb-8">
										...
										<div className="space-y-3">
											...
											<div className="flex items-center gap-3 text-gray-700">
												...
												<span>{selectedMember.slack}</span>
											</div>
										</div>
									</div>
											...
											{selectedMember.topSkills.map((skill, index) => (
												<div key={index} className="flex items-center gap-4">
													<div className="flex-1 bg-gray-200 rounded-full h-10 overflow-hidden">
														<div
															className={`${skill.color} h-full rounded-full flex items-center justify-center text-white font-semibold`}
															style={{ width: `${skill.rating}%` }}
														>
															{skill.rating}%
														</div>
													</div>
													<span className="font-medium text-gray-900 w-20 text-right">{skill.name}</span>
												</div>
											))}
										</div>
